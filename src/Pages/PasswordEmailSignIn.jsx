...
import { closePasswordModal, setPassword } from '../Features/SignInSlice'
import { closeSignInModal } from '../Features/SignInSlice'
import { openSignUpModal } from '../Features/SignUpSlice'
import { Login } from '../Services/emailSignInConfig'
function EmailSignIn() {
    const navigate = useNavigate()
    const dispatch = useDispatch();
    const [showPassword, setShowPassword] = useState(false)
    const passwordModal = useSelector((state) => state.SignInReducer.passwordArea)
    ...
    return (
        <>
            {passwordModal ? (
                <>
                    <div className={`fixed top-0 left-0 bottom-0 w-full h-full flex items-center justify-center ` + (passwordModal ? "z-50" : "z-40")}>
                        <div className='bg-black w-1/3 h-2/3 rounded-2xl relative flex flex-col'>
                            <div className='absolute flex justify-start items-center p-2 top-1'>
                                <button className='bg-181919 rounded-full p-1 duration-300 transition-all' onClick={() => dispatch(closePasswordModal())} >
                            ...
                            <div className='w-2/3 mx-auto flex flex-col gap-8 mt-16'>
                                <div className='flex items-center justify-start mb-3 w-full'>
                                    <h1 className='text-white text-4xl font-bold'>Şifreni Gir</h1>
                                </div>
                                <div className='flex flex-col items-start w-full gap-3 mb-32'>
                                    <div className="relative mb-3 w-full">
                                        <input value={email} disabled
                                            type="email" name='email' id="email"
                                            className="peer m-0 block h-[58px] w-full rounded border border-solid border-neutral-300 bg-gray bg-clip-padding px-3 py-4 text-xl font-normal leading-tight text-white transition duration-200 ease-linear placeholder:text-transparent focus:border-primary focus:pb-[0.625rem] focus:pt-[1.625rem] focus:text-white focus:outline-none peer-focus:text-primary dark:border-neutral-600 dark:text-neutral-200 dark:focus:border-primary dark:peer-focus:text-primary [&:not(:placeholder-shown)]:pb-[0.625rem] [&:not(:placeholder-shown)]:pt-[1.625rem]"
                                    ...
                                    <div className='w-full'>
                                        ...
                                        <div className='ps-1'>
                                            <button className='text-blue-500 text-sm'>Şifreni mi unuttun?</button>
                                        </div>
                                    </div>
                                ...
                                <div className='w-full'>
                                    ...
                                    <div className='ps-1 flex  gap-1 items-center '>
                                        <p className='text-gray text-base'>Henüz bir hesabın yok mu?</p><button className='text-blue-500 text-base' onClick={() => {
